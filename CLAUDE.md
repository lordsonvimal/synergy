...
**Before writing any UI code, read `packages/theme/tokens.css` for the full token definitions.** Also read `packages/theme/GUIDELINES.md` for rules on when to use each token. Never use raw hex values or default Tailwind palette (slate, gray, blue-500, etc.) — only use the token classes defined in the theme package.
